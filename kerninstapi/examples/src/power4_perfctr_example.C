...
int L1_miss_sample_callback(unsigned reqid, uint64_t time, 
                              const uint64_t *values, unsigned numvalues)
{
    cout << "@ Time = " << time << " cycles\n";
    for(int i = 0; i<numvalues; i++)
       cout << "   CPU " << i << ": L1 miss total = " << values[i] << endl;
}

int L1_load_sample_callback(unsigned reqid, uint64_t time, 
                             const uint64_t *values, unsigned numvalues)
{
    cout << "@ Time = " << time << " cycles\n";
    for(int i = 0; i<numvalues; i++)
       cout << "   CPU " << i << ": L1 load total = " << values[i] << endl;
}
...
int main(int argc, char **argv)
{
    ...
    int vt_id = add_vtimer(HWC_L1_DATA_MISS, entries, exits, false, 
                           L1_miss_sample_callback, 1000);
    if(vt_id < 0) {
       kmgr.detach();
       return vt_id;
    }
    ...
    // Insert vtimer at points
    vt_id = add_vtimer(HWC_L1_DATA_LOAD, entries, exits, false, 
                       L1_load_sample_callback, 1000);
    if(vt_id < 0) {
       kmgr.detach();
       return vt_id;
    }
