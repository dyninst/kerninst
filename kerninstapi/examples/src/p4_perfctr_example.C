...
int itlb_miss_sample_callback(unsigned reqid, uint64_t time, 
                              const uint64_t *values, unsigned numvalues)
{
    cout << "@ Time = " << time << " cycles\n";
    for(int i = 0; i<numvalues; i++)
       cout << "   CPU " << i << ": ITLB miss total = " << values[i] << endl;
}

int itlb_hit_sample_callback(unsigned reqid, uint64_t time, 
                             const uint64_t *values, unsigned numvalues)
{
    cout << "@ Time = " << time << " cycles\n";
    for(int i = 0; i<numvalues; i++)
       cout << "   CPU " << i << ": ITLB hit total = " << values[i] << endl;
}
...
int main(int argc, char **argv)
{
    ...
    int vt_id = add_vtimer(HWC_ITLB_MISS, entries, exits, false, 
                           itlb_miss_sample_callback, 1000);
    if(vt_id < 0) {
       kmgr.detach();
       return vt_id;
    }
    ...
    // Insert vtimer at points
    vt_id = add_vtimer(HWC_ITLB_HIT, entries, exits, false, 
                       itlb_hit_sample_callback, 1000);
    if(vt_id < 0) {
       kmgr.detach();
       return vt_id;
    }
