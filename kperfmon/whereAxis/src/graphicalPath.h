...
/* $Id: graphicalPath.h,v 1.1 1999/07/02 23:28:03 tamches Exp $ */
...
class whereNodeGraphicalPath {
 private:
   typedef where4tree<ROOTTYPE, childrenOracle> nodeType;
 ...
 public:
  ...
  ~whereNodeGraphicalPath() {}
   ...
   nodeType *getLastPathNode(nodeType *root) {
      return root->get_end_of_path(thePath);
   }

   const nodeType *getLastPathNode(const nodeType *root) const {
      return root->get_end_of_path(thePath);
   }

   nodeType *getParentOfLastPathNode(nodeType *root) {
