...
/* $Id: graphicalPath.C,v 1.2 2000/06/23 05:31:17 tamches Exp $ */

#include "graphicalPath.h"

template <class ROOTTYPE, class childrenOracle>
whereNodeGraphicalPath<ROOTTYPE, childrenOracle>::
whereNodeGraphicalPath(const whereNodePosRawPath &iPath,
		       const where4TreeConstants &tc,
		       const nodeType *rootPtr,
...
whereNodeGraphicalPath(int point_x, int point_y,
   ...
   while (true) {
      ...
      if (theItemLocation == -1)
         ...
         // Time to "recurse".
         const nodeType *childptr = rootPtr->getChildTree(childindex);
...
template <class ROOTTYPE, class childrenOracle>
whereNodeGraphicalPath<ROOTTYPE, childrenOracle> &
whereNodeGraphicalPath<ROOTTYPE, childrenOracle>::operator=
(const whereNodeGraphicalPath<ROOTTYPE, childrenOracle> &other) {
...
template <class ROOTTYPE, class childrenOracle>
bool whereNodeGraphicalPath<ROOTTYPE, childrenOracle>::
operator==(const whereNodeGraphicalPath<ROOTTYPE, childrenOracle> &other) const {
