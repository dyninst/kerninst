...
/* $Id: where4treeConstants.C,v 1.5 2004/04/22 21:23:32 mjbrim Exp $ */
...
where4TreeConstants::where4TreeConstants(Tcl_Interp *interp,
   ...
   rootTextFontStruct = Tk_GetFont(interp, theWindow, rootTextFontString);
//   rootTextFontStruct = Tk_GetFont(interp, theWindow, "*-Helvetica-*-r-*-14-*");
//   rootTextFontStruct = Tk_GetFont(interp, theWindow, "Helvetica 14");

   grayColor = Tk_GetColor(interp, theWindow, Tk_GetUid("gray"));
   if (grayColor == NULL)
      tclpanic(interp, "could not allocate gray color");

   const char *pinkName = Tcl_GetVar(interp, "pinkColor", TCL_GLOBAL_ONLY);
   if (pinkName == NULL)
      tclpanic(interp, "no tcl vrble pinkColor defined");
   pinkColor = Tk_GetColor(interp, theWindow, Tk_GetUid(pinkName));
   if (pinkColor == NULL)
   ...
   whiteColor = Tk_GetColor(interp, theWindow, Tk_GetUid("white"));
   if (whiteColor == NULL)
      tclpanic(interp, "could not allocate white color");

   const char *cornflowerBlueName = Tcl_GetVar(interp, "cornflowerBlueColor", TCL_GLOBAL_ONLY);
   if (cornflowerBlueName == NULL)
      tclpanic(interp, "no tcl vrble cornflowerBlueColor defined");
   cornflowerBlueColor = Tk_GetColor(interp, theWindow, Tk_GetUid(cornflowerBlueName));
   ...
   // Root Border
   rootNodeBorder = Tk_Get3DBorder(interp, theWindow, Tk_GetUid(pinkName));
   ...
   // Master listbox Font				  
   listboxFontStruct = Tk_GetFont(interp, theWindow, listboxTextFontString);
//   listboxFontStruct = Tk_GetFont(interp, theWindow, "*-Helvetica-*-r-*-12-*");
//   listboxFontStruct = Tk_GetFont(interp, theWindow, "Helvetica 12");

   // Master listbox Borders
   //listboxBorder = Tk_Get3DBorder(interp, theWindow, Tk_GetUid("cornflowerBlue"));
   listboxBorder = Tk_Get3DBorder(interp, theWindow, Tk_GetUid(cornflowerBlueName));
   if (rootNodeBorder == NULL) {
      cerr << interp->result << endl;
      exit(5);
   }
   ...
   // Master listbox Text
   values.foreground = whiteColor->pixel;
