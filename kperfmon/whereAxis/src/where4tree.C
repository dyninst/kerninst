...
/* $Id: where4tree.C,v 1.15 2004/04/22 21:23:30 mjbrim Exp $ */

/* ******************************************************************
 *
 * Notes on scrollbars and drawing speed and tk/tcl vs. C:
 *
 * 1. Drawing must be fast because some where axes (e.g. the search history graph of
 *    the performance consultant) are changing rapidly.
 *
...
void where4tree<NODEDATA, childrenOracle>::removeListbox() {
   listboxPixWidth = listboxFullPixHeight = listboxActualPixHeight = 0;
   theScrollbar.invalidate();
}

template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::rethinkListboxAfterResize1(const where4TreeConstants &tc) {
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::rethinkListboxAfterResize(const where4TreeConstants &tc) {
...
template <class NODEDATA, class childrenOracle>
int where4tree<NODEDATA, childrenOracle>::scroll_listbox(const where4TreeConstants &tc,
					 int deltaYpix) {
   // doesn't redraw
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::scroll_listbox(const where4TreeConstants &tc,
					  int listboxLeft,
					  int listboxTop,
					  int deltaYpix) {
...
template <class NODEDATA, class childrenOracle>
void where4tree<NODEDATA, childrenOracle>::rethink_listbox_dimensions(const where4TreeConstants &tc) {
   ...
   for (unsigned childlcv=0; childlcv < theChildren.size(); childlcv++) {
      ...
      where4tree<NODEDATA, childrenOracle> *childPtr = getChildTree(childlcv);
...
template <class NODEDATA, class childrenOracle>
void where4tree<NODEDATA, childrenOracle>::drawTriangle(const where4TreeConstants &tc,
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::
draw_listbox(Tk_Window theTkWindow,
   ...
   if (left > Tk_Width(theTkWindow)-1) return false; // everything's too far right
   if (left + listboxPixWidth - 1 < 0) return false; // everything's too far left
   if (top > Tk_Height(theTkWindow)-1) return false; // everything's too far down
   if (top + listboxActualPixHeight - 1 < 0) return false; // everything's too high
   ...
   if (getNumChildren() < childrenOracle::numChildren(getNodeData().getUniqueId())) {
      childrenOracle::addChildrenNow(this, tc);

      assert(getNumChildren() ==
             childrenOracle::numChildren(getNodeData().getUniqueId()) &&
             "draw_listbox(): childrenOracle::addChildrenNow() didn't validate the invariant; perhaps we have duplicate names (would inflate childrenOracle::numChildren())");
   ...
   for (unsigned childlcv=0; childlcv < theChildren.size(); childlcv++) {
      ...
      const where4tree<NODEDATA, childrenOracle> *childPtr = getChildTree(childlcv);
      if (!childPtr->anything2Draw)
         continue;
      ...
      const where4tree<NODEDATA, childrenOracle> *childNode =
         theChildren[childlcv].theTree;

      const NODEDATA &theChildNodeData = childNode->theNodeData;
      ...
      if (childNode->getNumChildren() > 0 ||
          childrenOracle::hasChildren(childNode->getNodeData().getUniqueId())) {
 ...
 */
template <class NODEDATA, class childrenOracle>
int where4tree<NODEDATA, childrenOracle>::
...
template <class NODEDATA, class childrenOracle>
int where4tree<NODEDATA, childrenOracle>::vert_pix_everything_below_root() const {
   return max(listboxActualPixHeight, allExpandedChildrenHeightAsDrawn);
}

template <class NODEDATA, class childrenOracle>
int where4tree<NODEDATA, childrenOracle>::entire_width(const where4TreeConstants &tc) const {
   return max(horiz_pix_everything_below_root(tc), getNodeData().getWidthAsRoot());
}

template <class NODEDATA, class childrenOracle>
int where4tree<NODEDATA, childrenOracle>::entire_height(const where4TreeConstants &tc) const {
 ...
 */
template <class NODEDATA, class childrenOracle>
int where4tree<NODEDATA, childrenOracle>::horiz_offset_to_expanded_child(const where4TreeConstants &tc,
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::
draw(Tk_Window theTkWindow,
     const where4TreeConstants &tc,
     Drawable theDrawable,
     int middlex, int topy,
        // describes position of root node
     bool rootOnly, bool listboxOnly) {
   // returns true iff oracle detecting that things were added or deleted, and
   // thus some higher authority should consider rethinking layout & redrawing.
   if (!anything2Draw)
      return false;

   const int maxWindowY = Tk_Height(theTkWindow) - 1;
   if (topy > maxWindowY)
      return false; // everything is too far down

   if (!listboxOnly)
      theNodeData.drawAsRoot(theTkWindow, theDrawable, middlex, topy);

   if (rootOnly || theChildren.size() == 0)
      return false; // no children to draw...
   ...
   if (listboxPixWidth > 0) {
	 ...
	 GC theGC = NODEDATA::getGCforListboxRay(theNodeData,
      ...
      if (draw_listbox(tc.theTkWindow, tc, theDrawable,
                       currentXpix, // left
                       childtopypix, // top
                       -1, -1)) {
         needsRethinkingAndRedrawing = true;

//           cout << "draw_listbox returned true; will need to rethink layout" << endl;
//           cout << "The parent node's name is \"" << getNodeData().getName() << "\""
//                << endl;
      }
   ...
   if (listboxOnly)
      return needsRethinkingAndRedrawing;
...
//            const int rayDestinationX = windowMaxX;
	 ...
	 XDrawLine(tc.display, theDrawable, lineGC,
      ...
      if (theChild->draw(tc.theTkWindow, tc, theDrawable,
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::
expandUnexpand1(const where4TreeConstants &tc,
   ...
   // If expanding, and some new children need to be added to the tree, then do so.
   if (expandFlag && getChildTree(childindex)->getNumChildren() <
       childrenOracle::numChildren(getChildTree(childindex)->getNodeData().getUniqueId())) {
      childrenOracle::addChildrenNow(getChildTree(childindex), tc);
   }
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::
explicitlyExpandSubchild(const where4TreeConstants &tc,
                         unsigned childindex,
                         bool force) {
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::explicitlyUnexpandSubchild(const where4TreeConstants &tc,
						      unsigned childindex) {
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::expandAllChildren(const where4TreeConstants &tc) {
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::unExpandAllChildren(const where4TreeConstants &tc) {
   // returns true iff any changes were made
   for (unsigned childlcv=0; childlcv < theChildren.size(); childlcv++)
...
template <class NODEDATA, class childrenOracle>
void where4tree<NODEDATA, childrenOracle>::rethinkAfterResize(const where4TreeConstants &tc,
   ...
   if (false && existslistbox && childrenWidthIfAllExpanded <= horizSpaceToWorkWithThisSubtree) {
...

template <class NODEDATA, class childrenOracle>
int where4tree<NODEDATA, childrenOracle>::point2ItemOneStepScrollbar(const where4TreeConstants &tc,
...
template <class NODEDATA, class childrenOracle>
int where4tree<NODEDATA, childrenOracle>::point2ItemOneStep(const where4TreeConstants &tc,
					    int xpix, int ypix,
...
template <class NODEDATA, class childrenOracle>
int where4tree<NODEDATA, childrenOracle>::point2ItemWithinListbox(const where4TreeConstants &tc,
...
template <class NODEDATA, class childrenOracle>
int where4tree<NODEDATA, childrenOracle>::substring2Path(whereNodePosRawPath &thePath,
                                                         const where4TreeConstants &tc,
                                                         const pdstring &str,
...
//   bool match = anything2Draw && testRootNode &&
//                str.prefix_of(getRootName());
   bool match = anything2Draw && testRootNode &&
                str == getRootName().substr(0, str.length());
   if (match) {
...
//         match = str.prefix_of(childName);
         match = (str == childName.substr(0, str.length()));
         ...
         if (theChild->substring2Path(thePath, tc, str, beginSearchFrom, false) != 0)
   ...
   for (unsigned j=0; j < numChildren; j++) {
      ...
      if (childIsExpanded) {
         ...
         int result = theChild->substring2Path(thePath, tc, str, beginSearchFrom, true);
         if (result == 0)
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::
fullName2Path(whereNodePosRawPath &thePath, // updated in place
              const pdstring &fullName,
              const where4TreeConstants &consts) {
   if (fullName.length() == 0)
      return false;
   
   // If the name begins with a slash, then search the children
   const char firstChar = fullName[0];
   if (firstChar == '/') {
      // Search children.  Load them into memory if needed.
      if (getNumChildren() == 0 &&
          childrenOracle::hasChildren(getNodeData().getUniqueId()))
         childrenOracle::addChildrenNow(this, consts);

      thePath.append(UINT_MAX);
      unsigned &i = thePath.getLastItem(); // the same vrble name as in the
         // for stmt below, that's ON PURPOSE!
      
//      const pdstring nameAfterSlash = pdstring(fullName.begin() + 1, fullName.end());
      const pdstring nameAfterSlash = fullName.substr(1, fullName.length()-1);
      for (i=0; i < theChildren.size(); ++i) {
         if (theChildren[i].theTree->fullName2Path(thePath, nameAfterSlash, consts))
            return true;
      }
      ...
      // Full name does not begin with a slash.

      // Is there a slash at all?
      unsigned ndx = fullName.find("/");
      if (ndx >= fullName.length())
         // no slash; we must match the entire 'fullName'
         ndx = fullName.length();
      
      pdstring firstComponent = fullName.substr(0, ndx);
         // starting at char 0, lasting for 'ndx' chars.  Does not include the slash.

      if (firstComponent != getRootName()) {
//         cout << "Sorry, " << firstComponent << " did not match "
//              << getRootName() << endl;

         return false;
      }
      
//      cout << "Matched " << firstComponent << endl;
      
      // Gather what's left to search:
      pdstring afterFirstComponent = fullName.substr(ndx, fullName.length() - ndx);
         // includes the slash (intentionally)

      if (afterFirstComponent.length() == 0) {
//         cout << "Nothing left to match; success!" << endl;
         return true;
      }
      else {
//         cout << "Continuing the search...now searching for "
//              << afterFirstComponent << endl;
         return fullName2Path(thePath, afterFirstComponent, consts);
      }
   }
}
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::
path2lbItemExpand(const where4TreeConstants &tc,
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::
expandEntirePath(const where4TreeConstants &tc,
                 const whereNodePosRawPath &thePath,
                 unsigned index) {
   // Same as path2lbItemExpand, except we won't expand a listbox item at the
   ...
   if (!allChildrenExpanded && !theChildren[childindex].isExplicitlyExpanded) {
      ...
      if (!explicitlyExpandSubchild(tc, childindex, false))
         // false --> don't force
         assert(false);
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::path2lbItemUnexpand(const where4TreeConstants &tc,
					       const whereNodePosRawPath &thePath,
					       unsigned pathIndex) {
   // returns true iff any changes were made
   assert(anything2Draw);
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::path2ExpandAllChildren(const where4TreeConstants &tc,
						  const whereNodePosRawPath &thePath,
						  unsigned pathIndex) {
   // returns true iff any changes.
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::path2UnExpandAllChildren(const where4TreeConstants &tc,
						    const whereNodePosRawPath &thePath,
						    unsigned pathIndex) {
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::
path2SelectUnSelect(bool selectFlag,
                    const where4TreeConstants &tc,
                    const whereNodePosRawPath &thePath,
                    unsigned pathIndex) {
   // returns true iff any changes were made.  Doesn't redraw.
   ...
   if (pathIndex < thePath.getSize()) {
      ...
      const unsigned thisItem = thePath[pathIndex];
      assert(getChildTree(thisItem)->anything2Draw);

      return getChildTree(thisItem)->path2SelectUnSelect(selectFlag,
                                                         tc, thePath, pathIndex+1);
   }
   else {
      if (selectFlag)
 ...
 */
template <class NODEDATA, class childrenOracle>
where4tree<NODEDATA, childrenOracle>::where4tree(const NODEDATA &iNodeData) :
   ...
   changesSinceLastSort = false;
}

template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::updateAnything2Draw1(const where4TreeConstants &tc) {
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::updateAnything2Draw(const where4TreeConstants &tc) {
...
template <class NODEDATA, class childrenOracle>
where4tree<NODEDATA, childrenOracle>::~where4tree() {
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::
alreadyExistsChildName(const pdstring &proposedNewChildName) {
   sortChildren(); // won't sort if not needed

   const bool assertSorted = false;
   
   if (assertSorted) {
      typename pdvector<childstruct>::const_iterator iter = theChildren.begin();
      typename pdvector<childstruct>::const_iterator finish = theChildren.end();
      if (iter != finish) {
         // assert sorted
         for (; iter != finish-1; ++iter) {
            // assert *(iter+1) isn't less than *iter
            assert(!sort_comparitor(*(iter+1), *iter));
         }
      }
      // reset iter!
      iter = theChildren.begin();
   }

   if (false) {
      // the slow way (while debugging some weird behavior of binary_search())
      typename pdvector<childstruct>::const_iterator iter = theChildren.begin();
      typename pdvector<childstruct>::const_iterator finish = theChildren.end();
      for (; iter != finish; ++iter) {
         const childstruct &cs = *iter;
     
         if (cs.theTree->getRootName() == proposedNewChildName)
            return true;
      }
      return false;
   }
      ...
      typename pdvector<childstruct>::const_iterator iter = theChildren.begin();
      typename pdvector<childstruct>::const_iterator finish = theChildren.end();
      return std::binary_search(iter, finish, proposedNewChildName,
                                search_comparitor());
   }
}

template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::addChild(where4tree *theNewChild,
...
template <class NODEDATA, class childrenOracle>
bool where4tree<NODEDATA, childrenOracle>::
deleteChild(where4tree *theChild,
            const where4TreeConstants &tc,
            bool rethinkLayoutNow,
            bool resortNow) {
   sortChildren(); // required for binary searching

   typename pdvector<childstruct>::iterator iter = std::lower_bound(theChildren.begin(),
                                                         theChildren.end(),
                                                         theChild->getNodeData().getName(),
                                                    search_comparitor());
   if (iter == theChildren.end() || iter->theTree != theChild) {
      cout << "WARNING: where axis not deleting item \""
           << theChild->getNodeData().getName() << "\"" << endl;
      cout << "because not found as a child of \""
           << getNodeData().getName() << "\", as expected!" << endl;

      return false;
         // don't forget to return; we don't want to execute *iter = xxx below
         // since iter may be end()!
   }

//     cout << "Where axis: successfully found, and about to delete item " << endl
//          << "\"" << theChild->getNodeData().getName() << "\" with the expected parent of"
//          << endl
//          << "\"" << getNodeData().getName() << "\"..." << endl;

   // destroy the child; this will recursively call destructors as appropriate
   delete iter->theTree;

   *iter = theChildren.back();
   theChildren.pop_back();
   changesSinceLastSort = true; // don't forget this, or next re-sort will do nothing!
   
   if (rethinkLayoutNow) {
      // could probably do just one of the two:
      rethink_all_expanded_children_dimensions(tc);
      rethink_listbox_dimensions(tc);
   }
...
template <class NODEDATA, class childrenOracle>
void where4tree<NODEDATA, childrenOracle>::
doneAddingChildren(const where4TreeConstants &tc, bool sortNow) {
...
template <class NODEDATA, class childrenOracle>
void where4tree<NODEDATA, childrenOracle>::sortChildren() {
   ...
   if (changesSinceLastSort && theChildren.size() > 1)
      std::sort(theChildren.begin(), theChildren.end(), sort_comparitor);
...
//           const bool result = (0==strcmp(getRootName().c_str(), name));
...
//     // Load children into memory if appropriate!
//     if (getNumChildren() == 0 &&
//         childrenOracle::hasChildren(getNodeData().getUniqueId()))
//        childrenOracle::addChildrenNow(this, consts);
...
template <class NODEDATA, class childrenOracle>
pdvector<const NODEDATA *> where4tree<NODEDATA, childrenOracle>::getSelections() const {
...
template <class NODEDATA, class childrenOracle>
void where4tree<NODEDATA, childrenOracle>::recursiveClearSelections() {
