...
extern "C" void* kmem_getpages(pgcnt_t npages, u_int flag);
extern "C" void  kmem_freepages(void *addr, pgcnt_t npages);
...
kptr_t allocate_from_kmem32(unsigned nbytes)
{
    ...
    rv = kmem_getpages(npages, KM_SLEEP);
...
void free_from_kmem32(kptr_t addr, unsigned nbytes)
{
    ...
    kmem_freepages((void *)addr, npages);
