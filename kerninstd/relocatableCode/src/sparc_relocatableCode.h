// sparc_relocatableCode.h

#ifndef _SPARC_RELOCATABLE_CODE_H_
#define _SPARC_RELOCATABLE_CODE_H_
...
class sparc_relocatableCode : public relocatableCode_t {
 public:
   
   explicit sparc_relocatableCode(const tempBufferEmitter &em);
   sparc_relocatableCode(Dummy);
   sparc_relocatableCode(XDR *);
   sparc_relocatableCode(const relocatableCode_t &src); // yes, we really do need this (nuts)
   ~sparc_relocatableCode();
   ...
   typedef sparc_tempBufferEmitter::setHiBSetOrAddLocation setHiBSetOrAddLocation;
   typedef sparc_tempBufferEmitter::setHiBSetBackpatchInfo setHiBSetBackpatchInfo;
...
obtainFullyResolvedCode(const pdvector<kptr_t> &chunkStartAddrs, // an entry per chunk
   ...
   // STEP 4.5: unresolvedLabelAddrsAsData
   {
      ...
      for (; iter != finish; ++iter) {
         ...
         sparc_instr *i = (sparc_instr *)offset2Insn(theInsnOffset, fullyResolvedCodeCache);
         *i = sparc_instr(valueToUse);
      }
   }
   // We *don't* clear unresolvedSymAddrs[] because this method may get called
   // again, with a different emitAddr!
   ...
   // Loop thru unresolvedSetHiBSetOrAddBackpatches:
   {
      pdvector<setHiBSetBackpatchInfo>::const_iterator iter =
         unresolvedSetHiBSetOrAddBackpatches.begin();
      pdvector<setHiBSetBackpatchInfo>::const_iterator finish =
         unresolvedSetHiBSetOrAddBackpatches.end();

      for (; iter != finish; ++iter) {
         const setHiBSetBackpatchInfo &info = *iter;
      
         const insnOffset labelOffset = labels.get(info.labelName);
         const kptr_t labelAbsoluteAddr = absolute_addr(labelOffset, chunkStartAddrs);
         const kptr_t newDestAddr = labelAbsoluteAddr + info.byteOffset;
      
         // patch up the sethi instruction:
         insnOffset setHiInsnOffset = labels.get(info.insnLocs.setHiLabelName);
         setHiInsnOffset.byteOffset += info.insnLocs.setHiInsnByteOffset;
         instr_t *setHiInsn = offset2Insn(setHiInsnOffset,
					  fullyResolvedCodeCache);

         sparc_reg rd = sparc_reg::g0;
         if (!((sparc_instr*)setHiInsn)->isSetHi(rd))
            assert(false);
         assert(rd != sparc_reg::g0);
         *setHiInsn = sparc_instr(sparc_instr::sethi,
				  sparc_instr::HiOf(), newDestAddr, rd);
      
         // patch up the bset or add instruction:
         insnOffset bsetOrAddInsnOffset = labels.get(info.insnLocs.bsetOrAddLabelName);
         bsetOrAddInsnOffset.byteOffset += info.insnLocs.bsetOrAddInsnByteOffset;
         instr_t *bsetOrAddInsn = offset2Insn(bsetOrAddInsnOffset,
					      fullyResolvedCodeCache);

         unsigned val = 0; // important to initialize this to zero
         sparc_reg rs1 = sparc_reg::g0;
         if (((sparc_instr*)bsetOrAddInsn)->isORImm(rs1, val, rd)) {
            *bsetOrAddInsn = sparc_instr(sparc_instr::logic, sparc_instr::logic_or,
					 false, // no cc
					 rd, rs1,
					 sparc_instr::lo_of_32(newDestAddr));
         }
         else {
            val = 0; // important to reset this
            if (((sparc_instr*)bsetOrAddInsn)->isAddImm(rs1, val, rd))
               *bsetOrAddInsn = sparc_instr(sparc_instr::add,
					    rs1,
					    sparc_instr::lo_of_32(newDestAddr),
					    rd);
            else
      ...
      // We don't clear unresolvedSetHiBSetOrAddBackpatches[] because
      // this method may get called again, with a different emitAddr!
   }
