// x86_relocatableCode.h

#ifndef _X86_RELOCATABLE_CODE_H_
#define _X86_RELOCATABLE_CODE_H_
...
class x86_relocatableCode : public relocatableCode_t {
 public:
   
   explicit x86_relocatableCode(const tempBufferEmitter &em);
   x86_relocatableCode(Dummy);
   x86_relocatableCode(XDR *);
   x86_relocatableCode(const relocatableCode_t &src); // yes, we really do need this (nuts)
   ~x86_relocatableCode();
...
obtainFullyResolvedCode(const pdvector<kptr_t> &chunkStartAddrs, // an entry per chunk
   ...
   // STEP 1: resolve branches to labels
   {
      ...
      for (; iter != finish; ++iter) {
         ...
         i->changeBranchOffset(delta - i->getNumBytes());
      }
   }
   ...
   // STEP 1.5: fix up unresolved *interprocedural* branches by name
   {
      ...
      for (; iter != finish; ++iter) {
         ...
         if (result.first)
            ...
            assert(!"Error: x86_relocatableCode::obtainFullyResolvedCode() - could not resolve interprocedural branch by name");
         instr_t *i = offset2Insn(branchinsn_off, fullyResolvedCodeCache);
            
         const kptr_t thisInsnAbsoluteAddr =
            absolute_addr(branchinsn_off, chunkStartAddrs);
         const kaddrdiff_t offset = destAbsoluteAddr - (thisInsnAbsoluteAddr + i->getNumBytes());
   ...
   // STEP 2: fix up unresolved calls-by-name.
   {
      ...
      for (; iter != finish; ++iter) {
         ...
         if (!addr.first) {
            cout << "Error: x86_relocatableCode::obtainFullyResolvedCode() - could not resolve call symbol named \""
                 << theSymName << "\"" << endl;
   ...
   // STEP 4 -- fix up unresolvedSymAddrs:
   {
      ...
      for (; iter != finish; ++iter) {
         ...
         if (iter->second == tempBufferEmitter::this_chunk_addr)
            ...
            if (!result.first) {
               cout << "Error: x86_relocatableCode::obtainFullyResolvedCode() - could not resolve symAddr symbol named \""
                    << theSymName << "\"" << endl;
               assert(false); // throw an exception instead
   ...
   // STEP 4.5: unresolvedLabelAddrsAsData
   {
      ...
      for (; iter != finish; ++iter) {
         ...
         assert(!"x86_relocatableCode::obtainFullyResolvedCode() - not supported on x86\n");
      }
   }
   // We *don't* clear unresolvedSymAddrs[] because this method may get called
   // again, with a different emitAddr!
