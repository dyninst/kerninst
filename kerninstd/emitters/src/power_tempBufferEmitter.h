// power_tempBufferEmitter.h

#ifndef _POWER_TEMPBUFFEREMITTER_H_
#define _POWER_TEMPBUFFEREMITTER_H_
#include "tempBufferEmitter.h"

class power_tempBufferEmitter :  public tempBufferEmitter { 
 ...
 private:
   ...
   void fixUpKnownCalls();
     ...
     // Prevent copying:
     power_tempBufferEmitter(const power_tempBufferEmitter &);
     power_tempBufferEmitter &operator=(const power_tempBufferEmitter &);

 public:
     power_tempBufferEmitter(const abi &);
   ...
   // emits the ld or ldx operation depending on the native pointer type
   void emitLoadKernelNative(reg_t &srcAddrReg1, reg_t &srcAddrReg2, reg_t &rd) ;
   void emitLoadDaemonNative(reg_t &srcAddrReg1, reg_t &srcAddrReg2, reg_t &rd);

   // emits the st or stx operation depending on the native pointer type
   void emitStoreKernelNative(reg_t &valueReg, 
			      reg_t &addressReg, signed offset);
   void emitStoreDaemonNative(reg_t &valueReg, 
			      reg_t &addressReg, signed offset);

   // emits the st or stx operation depending on the native pointer type
    void emitStoreKernelNative(reg_t &valueReg, 
			      reg_t &addressReg1, reg_t &addressReg2);
    void emitStoreDaemonNative(reg_t &valueReg, 
			      reg_t &addressReg1, reg_t &addressReg2);
