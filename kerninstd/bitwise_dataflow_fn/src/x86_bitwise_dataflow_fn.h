#ifndef _X86_BITWISE_DATAFLOW_FN_H_
#define _X86_BITWISE_DATAFLOW_FN_H_
...
#include "x86_mbdf_window.h"
#include "x86Traits.h"
#include "x86_reg_set.h"

struct XDR;

class x86_bitwise_dataflow_fn : public monotone_bitwise_dataflow_fn {
 public:
   static const x86_bitwise_dataflow_fn startAllRegs;
   static const x86_bitwise_dataflow_fn stopAllRegs;
   static const x86_bitwise_dataflow_fn passAllRegs;
   static const x86_bitwise_dataflow_fn dummy_fn;

 private:
   x86_mbdf_window data;
   bool Dummy;

   // slow ctors:
   x86_bitwise_dataflow_fn(const x86_mbdf_window &srcdata) :
      monotone_bitwise_dataflow_fn(), data(srcdata), Dummy(false) {}
   x86_bitwise_dataflow_fn(bool iThruBit, bool iGenBit) :
      monotone_bitwise_dataflow_fn(), 
      data(x86_mbdf_window(iThruBit, iGenBit)), Dummy(false) {}

 public:
   x86_bitwise_dataflow_fn(XDR *xdr) : 
      monotone_bitwise_dataflow_fn(), data(xdr), Dummy(false) {}
   bool send(XDR *xdr) const { return data.send(xdr); }

   // fast ctors:
   x86_bitwise_dataflow_fn(bool, bool, bool, bool) :
      monotone_bitwise_dataflow_fn(), Dummy(true) {}
      
   x86_bitwise_dataflow_fn() : monotone_bitwise_dataflow_fn(), Dummy(false) {}

   x86_bitwise_dataflow_fn(const x86_bitwise_dataflow_fn &src) :
      monotone_bitwise_dataflow_fn(), data(src.data), Dummy(src.Dummy) {}

   x86_bitwise_dataflow_fn(const monotone_bitwise_dataflow_fn &src) :
      monotone_bitwise_dataflow_fn(), 
      data(((x86_bitwise_dataflow_fn &)src).data), 
      Dummy(((x86_bitwise_dataflow_fn &)src).Dummy) {}

   x86_bitwise_dataflow_fn(functions iFn) : 
      monotone_bitwise_dataflow_fn(), 
   ...
   ~x86_bitwise_dataflow_fn() {}

   // fast assignment operator:
   x86_bitwise_dataflow_fn& operator=(const x86_bitwise_dataflow_fn &src) {
   ...
   bool operator==(const x86_bitwise_dataflow_fn &cmp) const {
   ...
   bool operator==(const monotone_bitwise_dataflow_fn &cmp) const{
      if (&cmp == this) return true;
      return data == ((x86_bitwise_dataflow_fn &)cmp).data;
   }
   ...
   compose(const monotone_bitwise_dataflow_fn &g) const {
      const x86_bitwise_dataflow_fn &g2 = (const x86_bitwise_dataflow_fn &)g;
      x86_mbdf_window *resultWindow = (x86_mbdf_window *)data.compose(g2.data);
      x86_bitwise_dataflow_fn *result = new x86_bitwise_dataflow_fn(*resultWindow);
      delete resultWindow;
      return result;
   }

   void compose_overwrite_param(monotone_bitwise_dataflow_fn &g) const {
      // calculates f o g (f is "this"; g is the parameter), and writes 
      // the result onto g.
      x86_bitwise_dataflow_fn &g2 = (x86_bitwise_dataflow_fn &)g;
      data.compose_overwrite_param(g2.data);
   }
   
   void mergeWith(const monotone_bitwise_dataflow_fn &g, 
                  mergeoperators mergeop){
      const x86_bitwise_dataflow_fn &g2 = (const x86_bitwise_dataflow_fn &)g;
   ...
   monotone_bitwise_dataflow_fn* 
   merge(const monotone_bitwise_dataflow_fn &g, mergeoperators mergeop) const {
      x86_bitwise_dataflow_fn *result = (x86_bitwise_dataflow_fn *)getDataflowFn(*this);
   ...
   pdstring sprintf(const pdstring &starts_banner, 
      ...
      return data.sprintf(starts_banner, stops_banner, starts_banner_first);
   }
   ...
   //The rest are not applicable to x86, but need to be declared, since
   //they are virtual

   unsigned getNumSavedWindows() const { return 0; }
   unsigned getNumLevels() const { return 1; }
   ...
   void setEachLevelTo(const regset_t *new_thru, const regset_t *new_gen) {
...
#endif /* _X86_BITWISE_DATAFLOW_FN_H_ */
