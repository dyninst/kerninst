...
instr_t* SpringboardLauncher::createSplicingInsn() const {
   ...
   assert(sparc_instr::inRangeOfBicc(from, sbItemAddr));
...
doInstallCommon(const regset_t *freeRegs,
   ...
   try {
      ...
      sparc_instr::getRangeOfBPcc(from,
      ...
      sparc_instr::getRangeOfBicc(from,
   ...
   if (sparc_instr::inRangeOfCall(from, to)) {
      if (no_regs_free) {
	 ...
	 const sparc_reg &tempreg = (const sparc_reg&)*iter;
	 instr_t *i = new sparc_instr(sparc_instr::mov,
				      sparc_reg::o7, tempreg);
	 springboardContents->appendAnInsn(i);
	 i = new sparc_instr(sparc_instr::callandlink,
			     to, sbItemAddr + 4);
	 springboardContents->appendAnInsn(i);
	 i = new sparc_instr(sparc_instr::mov,
			     tempreg, sparc_reg::o7);
	 springboardContents->appendAnInsn(i);
      }
   } 
   else {
	 ...
	 springboardContents->appendAnInsn(i);
	 sparc_instr::genJumpAndLink(springboardContents, to, sparc_reg::l0);
	 i = new sparc_instr(sparc_instr::restore);
	 springboardContents->appendAnInsn(i);
      }
      else {
	 sparc_reg& tempreg = (sparc_reg&)*(freeRegs->begin());
	 sparc_instr::genJumpAndLink(springboardContents, to, tempreg);
	 instr_t *i = new sparc_instr(sparc_instr::nop);
	 springboardContents->appendAnInsn(i);
      }
   }
