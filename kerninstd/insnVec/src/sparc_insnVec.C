// sparc_insnVec.C - implementation of arch-specific insnVec members for SPARC
...
sparc_insnVec::sparc_insnVec(const void *raw_insn_bytes, 
			     unsigned total_num_bytes) : insnVec_t() { 
   raw_initialize(raw_insn_bytes, total_num_bytes);
}

unsigned sparc_insnVec::getMemUsage_exceptObjItself() const {
   return sizeof(sparc_insnVec) +
          (the_insns.capacity() * sizeof(instr_t*)) + 
          (the_insns.size() * sizeof(sparc_instr));
}

void sparc_insnVec::reserveBytesHint(unsigned numbytes) {
...
unsigned sparc_insnVec::insnNdx2ByteOffset(unsigned ndx, 
...
unsigned sparc_insnVec::byteOffset2InsnNdx(unsigned byteoffset) const {
...
instr_t *sparc_insnVec::get_by_byteoffset(unsigned byteoffset) const {
   assert(byteoffset % 4 == 0);
   return the_insns[byteoffset / 4];
}

unsigned sparc_insnVec::offsetOfInsnContaining(unsigned byteoffset) const {
   return byteoffset & ~3;
}

void sparc_insnVec::raw_initialize(const void *raw_insn_bytes,
   ...
   char *raw_insn_ptr = (char *)raw_insn_bytes;
   for (; iter != finish; ++iter) {
      uint32_t raw = *(uint32_t*)raw_insn_ptr; // get raw insn
      raw_insn_ptr += 4; // go to next raw insn
      *iter = new sparc_instr(raw); 
...
sparc_insnVec::sparc_insnVec(XDR *xdr) {
   ...
   if (num_insns > 0) {
      ...
      for (; iter != finish; ++iter) {
	 *iter = (instr_t*) malloc(sizeof(sparc_instr));
...
void sparc_insnVec::shrinkToBytes(unsigned newNumBytes,
...
void *sparc_insnVec::reserveForManualOverwriteEnMasse(unsigned total_bytes) {
   the_insns.zap();
