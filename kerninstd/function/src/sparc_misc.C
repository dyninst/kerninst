...
static bool analyzeSliceForRegBoundsVia1CmpAndBranch(simplePathSlice_t &theSlice,
   ...
   if (theSlice.isEndOfIteration())
      return false;
   sparc_instr *lastInsn = (sparc_instr*)theSlice.getCurrIterationInsn();

   theSlice.advanceIteration();
   if (theSlice.isEndOfIteration())
      return false;
   sparc_instr *secToLastInsn = (sparc_instr*)theSlice.getCurrIterationInsn();
   ...
   update_fallthrough_bounds((sparc_instr::IntCondCodes)cond, val, min_val, max_val);
   return true;
}
